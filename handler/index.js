...
module.exports = async (client) => {
  ...
  const subcommandFiles = await globPromise(`${process.cwd()}/subcommands/*.js`);
  subcommandFiles.map((value) => {
    const subfile = require(value);
    const subsplitted = value.split("/");
    const subdirectory = subsplitted[subsplitted.length - 2];

    if (subfile.name) {
      const subproperties = { subdirectory, ...subfile };
      client.subcommands.set(subfile.name, subproperties);
    }
  });
